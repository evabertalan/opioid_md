...
set mol [mol new read_protein_mb_9_delmb_2_e162t_4x.psf type psf waitfor all] #REPLACE PSF FILE, delete comment after
set all [atomselect $mol "segname MON2 and name CA"]
set ref [atomselect $mol "segname MON2 and name CA" frame 0]
set sel [atomselect $mol "segname MON2 and name CA"]
mol addfile read_protein_mb_9_delmb_2_e162t_4.pdb waitfor all #REPLACE PDB FILE, delete comment after
bigdcd myrmsd dcd n1_to_n40.dcd #REPLACE DCD FILE, delete comment after
bigdcd_wait
quit
